...
The deployed files are in `site/`:
...
`deploy` uploads only these files to the bucket root. Package artifacts under
bucket prefixes are deployed by their owning repositories and must never be
deleted or synchronized by this repository.

## Source of Truth

- Use `README.md` for local setup and deployment requirements.
...
- Run `./qa-format` and `./run-qa-checks` after each change.
...
- Preserve the deployment script's allowlist of root UI assets.
