...
function getS3Data(marker, html) {
  ...
  // set loading notice
  $('#listing').html('<img src="./circle.gif" />');
  $.get(gcs_rest_url)
    .done(function (data) {
      ...
      html =
        typeof html !== 'undefined' ? html + prepareTable(info) : prepareTable(info);
...
function prepareTable(info) {
  ...
  content += `
      <tr>
        <th tabindex="0" role="button">Name</th>
        <th tabindex="0" role="button">Size</th>
        <th tabindex="0" role="button">Last Modified</th>
      </tr>
`;
  ...
  jQuery.each(files, function (idx, item) {
    ...
    if (item.Type === 'directory') {
      item.href = window.location.pathname + '?prefix=' + item.Key;
    } else {
      item.href = BUCKET_URL + item.Key;
    }
    var row = renderRow(item, cols);
    if (!EXCLUDE_FILE.includes(item.Key)) content += row;
  });
...
function renderRow(item, cols) {
  ...
  row += `<tr>
        ...
        <td class="detailsColumn" data-value="${item.Size || 0}">
          ${item.Size}
        </td>
  ...
  // row +=
  //   `<a class="${className}" href="` + item.href + '">' + item.keyText + "</a>";
